...
def clean_mimic_program(mimic_smtfile):
    ...
    raw = raw[raw.find("Bool"):] # need to adapt this for type changes
    let_values = []
    variables = 1
    found = True
    while found:
...
def mimic_program_global_accuracy(T):
    ...
    for i in tqdm(range(xtotest.shape[0])):
        ...
        if ground_truth == 7 or ground_truth == 1 or ground_truth == 0:  # add third class:
            total_predictions += 1
            program_outcome = classify_image(list(xtotest[i]))
            prediction = np.argmax(model_outcome[i])
            if program_outcome == 2 or program_outcome == 1: # adapt these lines for code for ternary classification
                truths += 1
            else:
                falses += 1
            if ((ground_truth == 7) and (program_outcome == 2)) or ((ground_truth == 1) and (program_outcome == 1)):
                right_predictions_acc += 1
            if ((prediction == 7) and (program_outcome == 2)) or ((prediction == 1) and (program_outcome == 1)):
