...
class TrainingInstance:
    ...
    def evaluate_model(self, verbose = 0):
        ...
        scores = [self.distance(self.predictions[i], self.y_test[i]) for i in range(len(self.predictions))]
        ...
        offending_inds = list(reversed(sorted([i for i in range(len(self.predictions))], key=lambda x: scores[x])))
        ...
        if verbose > 0:
            ...
            for ind in offending_inds[:num_challenging]:
                ...
                print("Prediction: {}".format(self.get_prediction(self.predictions[ind])))
    ...
    def make_constraints(self, constrainst_file="allConstraints.txt"):
        counter = 0
        g = open(constrainst_file, "w")
